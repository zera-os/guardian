...
std::vector<uint8_t> base58_decode(const std::string &encoded)
{
    ...
    for (int i = zeros; i < size; ++i)
    {
        ...
        if (pos == std::string::npos)
        {
            // Invalid character found, return empty vector
            std::cerr << "ERROR: Invalid base58 character found at position " << i 
                      << ": '" << encoded[i] << "' (ASCII: " << static_cast<int>(static_cast<unsigned char>(encoded[i])) << ")" << std::endl;
            std::cerr << "Input string: \"" << encoded << "\"" << std::endl;
